...
class BookReviewCommentControllerTest extends RestDocumentTest {
    ...
    void successFindAllByBookReviewID() throws Exception {
        // given
        when(bookReviewCommentService.findByBookReviewId(any()))
                .thenReturn(
                        List.of(
                                new BookReviewCommentResponse(
                                        5L,
                                        new Writer(UUID.randomUUID(), "작성자 이름"),
                                        "댓글 내용",
                                        LocalDateTime.now()),
                                new BookReviewCommentResponse(
                                        6L,
        ...
        // when
        ResultActions perform =
                mockMvc.perform(get("/book-review-comments").param("bookReviewId", "5"));
        // then
