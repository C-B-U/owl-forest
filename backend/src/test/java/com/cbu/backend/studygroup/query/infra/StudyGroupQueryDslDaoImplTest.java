...
class StudyGroupQueryDslDaoImplTest {
    ...
    class FindDetail {
        ...
        void successRun() throws Exception {
            ...
            StudyGroup studyGroup = StudyGroupFixture.SAMPLE1.toEntity(account2.getId(), List.of(account1.getId(), account3.getId()));
            ...
            assertThat(studyGroupInfo.getStudyGroupStatus()).isEqualTo(studyGroup.getStudyGroupStatus().toString());
            assertThat(studyGroupInfo.getName()).isEqualTo(studyGroup.getName());
            assertThat(studyGroupInfo.getLikeCount()).isEqualTo(studyGroup.getLikeCount().getMemberIds().size());
            assertThat(studyLeaderInfo.getId().toString()).isEqualTo(studyGroup.getStudyGroupMember().getLeaderId().getId().toString());
            assertThat(studyParticipants.size()).isEqualTo(studyGroup.getStudyGroupMember().getParticipantIds().size());
        }
    }
    ...
    class FindList {
        ...
        void successByCreatedAt() throws Exception {
            ...
            StudyGroup studyGroup1 = StudyGroupFixture.SAMPLE1.toEntity(account1.getId(), List.of(account2.getId()));
            StudyGroup studyGroup2 = StudyGroupFixture.SAMPLE1.toEntity(account2.getId(), List.of(account3.getId(), account1.getId()));
            ...
            //when
            List<StudyGroupResponse> result = studyGroupQueryDslDao.findAllStudyGroup(new Pageable() {
                @Override
