...
public class StudyCrewMember {
    ...
    @ElementCollection
    @CollectionTable(name = "study_crew_participants",
    ...
    private void checkParticipantNotDuplicated(List<AccountNo> studyCrewParticipants) {
        if (studyCrewParticipants.size() != studyCrewParticipants.stream().distinct().mapToInt(o -> 1).sum()) {
            throw new RuntimeException("중복 멤버가 있습니다.");
        }
    }

    private void checkLeaderInParticipants(AccountNo studyCrewLeaderId, List<AccountNo> studyCrewParticipantIds) {
        if (studyCrewParticipantIds.stream().anyMatch(participantId -> participantId.equals(studyCrewLeaderId))) {
