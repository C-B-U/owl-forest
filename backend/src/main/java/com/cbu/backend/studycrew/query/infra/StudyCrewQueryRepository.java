...
public class StudyCrewQueryRepository {
    ...
    public StudyCrew getStudyCrew(StudyCrewNo id) {
        return queryFactory
                ...
                .where(studyCrew.id.eq(id))
                .fetchOne();
    }

    public List<StudyCrew> getStudyCrewSortByCreatedAt(SortDirection sortDirection, Pageable pageable) {
        return getAllStudyCrew(getOrderSpecifier(studyCrew.baseTime.createdAt, sortDirection), pageable);
    }

    public List<StudyCrew> getStudyCrewSortByLikeCount(SortDirection sortDirection, Pageable pageable) {
        return getAllStudyCrew(getOrderSpecifier(studyCrew.likeCount.count, sortDirection), pageable);
    }

    private OrderSpecifier getOrderSpecifier(ComparableExpressionBase field, SortDirection sortDirection) {
