...
public class ReplyController {
  ...
  public ResponseEntity<Void> create(
      @RequestBody ReplyRequest dto) { // @RequestBody : http 요청의 본문 body 변환 후 파라미터로 전달
    replyService.create(dto);
    return ResponseEntity.status(HttpStatus.CREATED) // 이미 생성된 경우는 응답코드 반환,
        .build();
  }
  // 단일 조회
  @GetMapping("{id}")
  ...
  public ResponseEntity<List<ReplyResponse>> getAllOld() {
    return ResponseEntity.ok(replyService.findAllOld());
  }
  // 목록 조회 (최신순)
  @GetMapping("recent")
  public ResponseEntity<List<ReplyResponse>> getAllRecent() {
    return ResponseEntity.ok(replyService.findAllRecent());
  }
  // 키워드로 검색
  @GetMapping("search")
  public ResponseEntity<List<ReplyResponse>> getByKeyword(
      @RequestParam(value = "keyword") String keyword) {
    return ResponseEntity.ok(replyService.findByKeywordContains(keyword));
  }
