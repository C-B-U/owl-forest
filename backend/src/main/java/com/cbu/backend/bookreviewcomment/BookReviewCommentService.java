...
public class BookReviewCommentService {
    ...
    public Long save(BookReviewCommentRequest req) {
        ...
        return bookReviewCommentRepository
                .save(bookReviewCommentMapper.toEntity(req, bookReview, writer))
                .getId();
    }

    public List<BookReviewCommentResponse> findByBookReviewId(Long bookReviewId) {
        return bookReviewCommentRepository.findAllByBookReviewId(bookReviewId).stream()
                .map(bookReviewCommentMapper::toDto)
                .toList();
    }
}
