...
public class BookReviewQueryController {
    ...
    public ResponseEntity<List<BookReviewSummaryResponse>> getAll(
            @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable, @RequestParam(required = false) BookReviewQueryOption param) {
    ...
    public ResponseEntity<BookReviewResponse> getById(@PathVariable String id) {
        BookReviewResponse result =
                bookReviewDao.findResponseById(new BookReviewNo(id)).orElseThrow(EntityNotFoundException::new);
