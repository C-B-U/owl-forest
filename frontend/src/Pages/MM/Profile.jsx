...
const LeftWrap = styled.div`
  ...
  text-align: center;
  @media (max-width: 60rem) {
    // 창화면이 커지면 자신의 프로필만 나타나게 함.
    text-align: center;
  }
`;

const RightWrap = styled.div`
  // Right로 오른쪽에 부원리스트 화면 부분 나타내는 영역임
  flex-grow: 5;
  margin-left: 1rem;
  position: relative;

  @media (max-width: 60rem) {
    // 창화면 작아지면 프로필 수정 영역 안보이게 함.
    display: none;
  }
`;
...
const Intro = styled.div`
  // introduction 줄여서 intro ->한 줄 소개하는 영역
  width: 20rem;
  height: 5rem;
...
const TechMsg = styled.div`
  // TechMessage줄임. 화면상에 TECH STACK을 표시하는 텍스트 영역
  font-size: 20px;
  font-family: sans-serif;
`;
const TechMsgWrap = styled.div`
  // TechMsg를 감싸줌
  width: 100%;
  margin: 1rem 0;
  font-size: 20px;
...
const EditPro = styled(InputStyle)`
  // 프로필 편집 input박스
        ...
        <Wrap>
          <LeftWrap>
            ...
            <TechBox>
              ...
              <Tech>C++</Tech>
              <Tech>React</Tech>
              <Tech>SpringBoot</Tech>
              <Tech>Amazon AWS</Tech>
              <Tech>Kotlin</Tech>
            </TechBox>
          </LeftWrap>
          <RightWrap>
